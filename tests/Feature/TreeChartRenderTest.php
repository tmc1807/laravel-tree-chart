...
it('renders the sticky scrollbar strip when scrollable', function () {
    $html = view('tree', [
        'nodes' => [['id' => 'a', 'label' => 'Alpha']],
    ...
    expect($html)
        ->toContain('data-tc-scrollbar')
        ->toContain('tc-tree-scrollbar-thumb');
});

it('skips the sticky scrollbar strip when disabled or not scrollable', function () {
    $disabled = view('tree', [
        'nodes' => [['id' => 'a', 'label' => 'Alpha']],
        'options' => ['scrollable' => true, 'sticky_scrollbar' => false],
    ])->render();

    $nonScrollable = view('tree', [
        'nodes' => [['id' => 'a', 'label' => 'Alpha']],
        'options' => ['scrollable' => false],
    ])->render();

    expect($disabled)
        ->not->toContain('data-tc-scrollbar');
    expect($nonScrollable)
        ->not->toContain('data-tc-scrollbar');
});
